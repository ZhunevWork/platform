        ...
        <form method="POST" action="{{ route("admin.orders.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('apartament') ? 'is-invalid' : '' }}" name="apartament_id" id="apartament_id">
                    @foreach($apartaments as $id => $entry)
                        <option value="{{ $id }}" {{ old('apartament_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('presentation') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="presentation" id="presentation" value="1" {{ old('presentation', 0) == 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('worked') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="worked" id="worked" value="1" {{ old('worked', 0) == 1 ? 'checked' : '' }}>
